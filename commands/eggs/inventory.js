    ...
    run: async (client, message, args, egg, Discord) => {
        ...
        if (args[0]) {
            ...
            egg.query(`SELECT * FROM inventory WHERE guild = ${guild} AND userid = ${user.id}`, (err, rows) => {
                if (err) throw err;
                if (rows.length === 0) {
                    message.channel.send(`I think ${user.username} is up to something. He has nothing in his inventory!`).then(m => m.delete({
                        timeout: 5000
                    }));

                } else {

                    if (!rows[0].Chicken && !rows[0].Farm && !rows[0].Duck && !rows[0].Frog) return message.channel.send(`${user.username} has nothing in their inventory!`)

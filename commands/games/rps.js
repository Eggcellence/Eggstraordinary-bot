...
module.exports = {
    ...
    run: async (client, message, args, egg, Discord) => {
        ...
        message.channel.send(`This game costs \`${EntryPrice}\` 🥚 to enter. If you win you'll receive \`10\` 🥚 if you win. \nAre you sure you would like to continue? **(Y/N)**`).then(async (m)=> {
                    ...
                    egg.query(`SELECT * FROM UsersEggs WHERE guild = ${message.guild.id} AND userid = ${message.author.id}`, async (err, rows) => {
                        if(rows[0].eggs < EntryPrice) {
                            ...
                            message.channel.send('Rock, paper or scissors').then(() => {
                                collector.stop();
                                egg.query(`UPDATE UsersEggs SET eggs = eggs - ${EntryPrice} WHERE guild = ${message.guild.id} AND userid = ${message.author.id}`, (err, rows) => {
                                    if(err) throw err;
